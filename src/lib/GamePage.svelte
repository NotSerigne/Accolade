...
{:else}
    <div class="game-page" style:--game-bg={`url('${heroBackground(game)}')`}>
        ...
        <!-- Navigation : Absolue pour ignorer la gouttière de la scrollbar -->
        <div class="game-nav">
            <button class="nav-btn" disabled={!prevGame} onclick={() => prevGame && goToGame(prevGame.steam_id)}>
            ...
            <button class="nav-btn" disabled={!nextGame} onclick={() => nextGame && goToGame(nextGame.steam_id)}>
                    ...
                    <div class="progress-card">
                        <div class="progress-left">
                            <div class="progress-stats">
                                <span class="main-count"><strong>{unlockedCount}</strong> <span class="slash">/ {totalCount}</span></span>
                                                ...
                                                <div class="ach-footer-row">
                                                    {#if ach.completionpercentage}
                                                        <div class="rarity-info">
                                                            <span class="rarity-tag" style:background={palette.badgeBg} style:color={palette.badgeColor}>
                                                                {rarityLabel(ach.completionpercentage)} | {parseFloat(ach.completionpercentage).toFixed(1)}%
                                                            </span>
                                                            <div class="rarity-progress-track">
                                                                <div class="rarity-progress-fill" style:background={palette.fill} style:width={`${Math.min(parseFloat(ach.completionpercentage), 100)}%`}></div>
                                                            </div>
                                                        </div>
                                                    {/if}
                                                    ...
                                                    <div class="ach-actions">
                                                        ...
                                                        <button class="action-btn" onclick={() => searchGuide(game.name, ach.name || ach.key)}>
                                                            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
    ...
    .game-page {
        ...
        background-color: #080a0f; background-image: var(--game-bg);
    ...
    .game-scroll-area::-webkit-scrollbar { width: 6px; }
    .game-scroll-area::-webkit-scrollbar-thumb { background: rgba(255, 255, 255, 0.15); border-radius: 10px; }
    .game-scroll-area::-webkit-scrollbar-track { background: transparent; }

    .bg-overlay {
        position: fixed; inset: 0;
        background: linear-gradient(180deg, rgba(8, 10, 15, 0.2) 0%, rgba(8, 10, 15, 0.85) 35%, rgba(8, 10, 15, 0.98) 100%);
        z-index: -1;
        pointer-events: none;
    }
    ...
    .loading-state, .empty-state { height: 200px; }
    ...
    .nav-btn:hover:not(:disabled) { background: rgba(255,255,255,0.08); color: #fff; border-color: rgba(255,255,255,0.12); }
    .nav-btn:disabled { opacity: 0.2; cursor: default; }
    .nav-label { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .nav-index { font-size: 12px; color: #444; font-weight: 700; letter-spacing: 0.5px; }

    /* Header */
    .game-header { padding: 8px 0 32px; flex-shrink: 0; }
    .game-title-row { display: flex; align-items: center; justify-content: space-between; margin-bottom: 24px; }
    .game-title { font-size: 42px; font-weight: 800; color: #fff; letter-spacing: -1px; }
    .emulator-badge {
        font-size: 10px; font-weight: 700; color: #fff;
        background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.1);
        border-radius: 20px; padding: 4px 14px; text-transform: uppercase; letter-spacing: 1.2px;
    }

    /* Progress Card */
    .progress-card {
        background: rgba(13, 20, 28, 0.85); border: 1px solid rgba(255,255,255,0.04);
        border-radius: 16px; padding: 24px 32px; display: flex; gap: 48px;
        backdrop-filter: blur(12px); box-shadow: 0 8px 32px rgba(0,0,0,0.4);
    }
    .progress-left { flex: 1; display: flex; flex-direction: column; gap: 16px; }
    ...
    .progress-right { display: flex; gap: 40px; border-left: 1px solid rgba(255,255,255,0.08); padding-left: 48px; }
    .stat-item { display: flex; flex-direction: column; gap: 6px; }
    .stat-label { font-size: 11px; font-weight: 700; color: #6a7080; letter-spacing: 1.5px; }
    .stat-value { font-size: 24px; font-weight: 800; color: #fff; }
    .stat-value.highlight { color: #3ddc84; }

    /* Main Section */
    .main-section {
        flex: 1; display: flex; flex-direction: column; overflow: hidden;
        background: rgba(13, 20, 28, 0.45); border: 1px solid rgba(255, 255, 255, 0.03);
        border-radius: 20px 20px 0 0; backdrop-filter: blur(8px);
        min-width: 0;
    }
    ...
    .controls-bar {
        ...
        padding: 20px 28px; border-bottom: 1px solid rgba(255,255,255,0.04);
        flex-shrink: 0;
        overflow-x: auto;
    ...
    .filter-tabs { display: flex; background: rgba(0,0,0,0.3); border-radius: 10px; padding: 4px; gap: 4px; }
    .tab {
    ...
    .search-wrap {
        flex: 1; max-width: 440px; height: 38px;
        background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.05);
        border-radius: 10px; display: flex; align-items: center; gap: 12px; padding: 0 14px;
    }
    ...
    /* Achievements Grid */
    .achievements-list { flex: 1; padding: 24px 28px; }
    .ach-grid { display: flex; flex-direction: column; gap: 14px; }

    .ach-card {
        background: rgba(8, 12, 18, 0.6); border: 1px solid rgba(255,255,255,0.05);
        border-radius: 14px; transition: all 0.25s; position: relative;
    }
    .ach-card:hover { transform: translateY(-1px); background: rgba(12, 18, 26, 0.8); border-color: rgba(255,255,255,0.1); }
    .ach-card.is-unlocked { border-color: rgba(255,255,255,0.08); }
    .ach-card-inner { display: flex; gap: 16px; padding: 10px 16px; height: 88px; box-sizing: border-box; }

    .ach-icon-box {
        width: 68px; height: 68px; border-radius: 10px; overflow: hidden; flex-shrink: 0;
        background: #10141b; border: 1px solid rgba(255,255,255,0.05);
    }
    .ach-icon-box img { width: 100%; height: 100%; object-fit: cover; }
    .ach-icon-box img.muted-icon { opacity: 0.35; filter: grayscale(0.8); }
    .ach-placeholder { font-size: 28px; display: flex; align-items: center; justify-content: center; height: 100%; }
    ...
    .ach-footer-row { display: flex; align-items: center; justify-content: space-between; min-height: 20px; }
    .rarity-info { display: flex; align-items: center; gap: 12px; }
    .rarity-tag {
        font-size: 9px; font-weight: 800; padding: 2px 8px; border-radius: 4px;
        text-transform: uppercase; letter-spacing: 0.8px;
    }
    .rarity-progress-track { width: 100px; height: 4px; background: rgba(255,255,255,0.05); border-radius: 3px; overflow: hidden; }
    .rarity-progress-fill { height: 100%; border-radius: 3px; }

    /* Actions on hover */
    .ach-actions { display: flex; gap: 8px; opacity: 0; transition: opacity 0.2s; position: absolute; right: 16px; bottom: 10px; }
    .ach-card:hover .ach-actions { opacity: 1; }
    .action-btn {
        background: rgba(0,0,0,0.6); border: 1px solid rgba(255,255,255,0.1);
        border-radius: 6px; padding: 4px 10px; font-size: 10px; color: #ccc; font-weight: 700;
        cursor: pointer; display: flex; align-items: center; gap: 6px; transition: all 0.2s;
        backdrop-filter: blur(4px);
    }
    .action-btn:hover { background: rgba(255,255,255,0.1); color: #fff; border-color: rgba(255,255,255,0.2); }
    .action-btn.is-copied { background: rgba(61, 220, 132, 0.2); color: #3ddc84; border-color: rgba(61, 220, 132, 0.4); }

    .ach-card.jump-flash { animation: achJumpFlash 0.85s ease; }
    @keyframes achJumpFlash {
        0% { transform: scale(1.02); background: rgba(0, 102, 204, 0.15); border-color: rgba(0, 102, 204, 0.4); }
        100% { transform: scale(1); background: rgba(8, 12, 18, 0.6); }
    }

    .is-secret-hidden { opacity: 0.3; filter: blur(2px); }
</style>
