<script lang="ts">
	...
	$effect(() => {
		if (draft.language !== get(settings).language) {
			settings.update((s) => ({ ...s, language: draft.language }));
		}
	});
